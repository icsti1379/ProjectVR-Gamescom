...
namespace VR
{
    ...
    public class VRTK_SnapDropZone : MonoBehaviour
    {
        ...
        [Tooltip("A specified VR_PolicyList to use to determine which interactable objects will be snapped to the snap drop zone on release.")]
        public VR_PolicyList validObjectListPolicy;
        ...
        private VRTK_InteractableObject ValidSnapObject(GameObject checkObject, bool grabState)
        {
            ...
            return (ioCheck && ioCheck.IsGrabbed() == grabState && !VR_PolicyList.Check(checkObject, validObjectListPolicy) ? ioCheck : null);
        }
        ...
        private void CreateHighlightersInEditor()
        {
            ...
            if (VR_SharedMethods.IsEditTime())
            {
        ...
        private void InitialiseHighlighter()
        {
            ...
            {
                VR_SharedMethods.CloneComponent(existingHighlighter, highlightObject);
            }
            ...
            if (objectHighlighter.UsesClonedObject())
            {
                ...
                {
                    if (!VR_PlayerObject.IsPlayerObject(renderer.gameObject, VR_PlayerObject.ObjectTypes.Highlighter))
                    {
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            {
                var boxSize = VR_SharedMethods.GetBounds(highlightObject.transform).size * 1.05f;
